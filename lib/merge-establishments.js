const merge = (transaction, fromEstablishmentId, toEstablishmentId) => {
    ...
    .then(profilesNeedingPerms => {
      ...
      return transaction
        ...
        .then(() => {
          ...
          return transaction.insert(mergeLog).into('establishment_merge_log');
        });
    })

    .then(() => {
      // move pils to target establishment
      return transaction('pils')
...
const revertMerge = (transaction, fromEstablishmentId, toEstablishmentId) => {
  return transaction
    .from('establishment_merge_log')
    ...
    .then(mergeLog => {
      ...
      const removePermsQuery = transaction
        .from('permissions')
      ...
      const revertPilsQuery = transaction('pils')
      ...
      const revertProjectsQuery = transaction('projects')
        .update({ establishment_id: fromEstablishmentId })

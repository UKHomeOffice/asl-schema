...
describe('Profile model', () => {
  ...
  describe('search', () => {
    it('can search on full name', () => {
      ...
      return Promise.resolve()
        .then(() => Profile.searchAndFilter(opts))
        .then(profiles => {
    ...
    it('can search ASRU users on full name', () => {
      ...
      return Promise.resolve()
        .then(() => Profile.searchAndFilterAsru(opts))
        .then(profiles => {
          assert.deepEqual(profiles.total, 1);
  ...
  describe('scoped methods', () => {
    ...
    it('returns all related establishments if no establishmentId is passed', () => {
      const notEstablishmentScoped = Profile.scopeSingle({
        id: 'e7d0a987-62b5-48b5-8f27-9687b7a55cae'
      });
    ...
    it('returns only the scoped establishment if establishmentId is passed', () => {
      const establishmentScoped = Profile.scopeSingle({
        id: 'e7d0a987-62b5-48b5-8f27-9687b7a55cae',
        establishmentId: 8201
      });
    ...
    describe('getNamedProfiles', () => {
      it('only returns named people and own profile', () => {
        ...
        return Promise.resolve()
          .then(() => Profile.query().where({ email: expected[0] }))
          .then(profiles => profiles[0])
          .then(profile => Profile.getNamedProfiles({
            userId: profile.id,
            establishmentId: 8201
          }))
          .then(({ profiles }) => {
      ...
      describe('multi-establishment users', () => {
        ...
        it('excludes named people at other establishments', () => {
          return Promise.resolve()
            .then(() => Profile.query().where({ email: 'super@man.com' }))
            .then(profiles => profiles[0])
            .then(profile => Profile.getNamedProfiles({
              userId: profile.id,
              establishmentId: 8202
        ...
        it('includes named people at the same establishment', () => {
          return Promise.resolve()
            .then(() => Profile.query().where({ email: 'vincent@malloy.com' }))
            .then(profiles => profiles[0])
            .then(profile => Profile.getNamedProfiles({
              userId: profile.id,
              establishmentId: 8201
    ...
    describe('getNamed', () => {
      ...
      it('can retrieve the profile of a named person', () => {
        ...
        return Promise.all([
          Profile.query().where({ email }),
          Profile.query().where({ email: namedPersonEmail })
        ])
      ...
      it('cannot retrieve the profile of non named person', () => {
        ...
        return Promise.all([
          Profile.query().where({ email }),
          Profile.query().where({ email: requestedProfileEmail })
        ])
  ...
  describe('ASRU roles', () => {
    ...
    beforeEach(() => {
      return Profile.query().insertGraph([
        inspector, licensing, asruAdmin, asruSupport, asruRops
      ]);
    });

    it('strips inspector role when removing ASRU role', () => {
      return Profile.query().patchAndFetchById(inspector.id, { asruUser: false })
        .then(profile => {
    ...
    it('strips licensing officer role when removing ASRU role', () => {
      return Profile.query().patchAndFetchById(licensing.id, { asruUser: false })
        .then(profile => {
    ...
    it('strips admin role when removing ASRU role', () => {
      return Profile.query().patchAndFetchById(asruAdmin.id, { asruUser: false })
        .then(profile => {
    ...
    it('strips support role when removing ASRU role', () => {
      return Profile.query().patchAndFetchById(asruSupport.id, { asruUser: false })
        .then(profile => {
    ...
    it('strips rops role when removing ASRU role', () => {
      return Profile.query().patchAndFetchById(asruRops.id, { asruUser: false })
        .then(profile => {
